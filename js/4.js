function mapper(rules) {
  return function (item) {
    return rules.reduce((acc, [srcField, newFieldName, transformFn]) => {
      const newValue = transformFn
        ? transformFn(item[srcField])
        : item[srcField];

      acc[newFieldName] = newValue;
...
// Тестові дані та правила перетворення
let testData3 = [
  {
    name: "Vasya",
...
// Застосовуємо функцію перетворення
